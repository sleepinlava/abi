...
| 文件 | 用途 |
| --- | --- |
| `skills/autoplasm_agent/SKILL.md` | 仓库级 operator skill，说明 agent 如何安全使用 CLI、配置、registry、资源检查和 provenance。 |
| `skills/{tool}/SKILL.md` | 每个注册外部工具一份 skill，说明工具何时被选中、需要哪些输入、在哪个 mamba 环境运行、命令模板如何渲染、失败时如何排查。 |
| `skills/README.md` | 当前文件，作为 skills 目录的维护说明、详细索引和同步清单。 |
...
每个 `skills/{tool}/SKILL.md` 应包含以下小节：
...
| `Command Template` | 与 `config/tool_registry.yaml` 完全一致的 command template。 |
...
以下索引来自当前 `config/tool_registry.yaml` 和 `autoplasm list-tools` 输出。
...
| `fastplong` | `qc` | optional | required | `autoplasm-qc` | [fastplong](fastplong/SKILL.md) |
...
1. Update `config/tool_registry.yaml`.
2. Update the matching `skills/{tool}/SKILL.md`.
3. Update `envs/*.yml` if executable availability changes.
4. Update [../docs/tool_matrix.md](../docs/tool_matrix.md) if category, default status, or support status changes.
5. Run `autoplasm list-tools` and check the table still matches this README.

When changing CLI behavior:

1. Update [../README.md](../README.md).
2. Update [../docs/cli_usage.md](../docs/cli_usage.md).
...
3. Create `skills/{tool}/SKILL.md` using the required section structure above.
