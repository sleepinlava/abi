...
This module defines ``Protocol`` classes that plugins implement. A
...
* ``ABIPublishedOutputsPlugin`` -- optionally exposes stable final artifacts
  through every runtime transport.
"""
...
@runtime_checkable
class ABIResultValidationPlugin(Protocol):
    """Optional capability for plugin-specific result-directory validation."""
...
@runtime_checkable
class ABIPublishedOutputsPlugin(Protocol):
    """Optional extension for final plugin artifacts exposed by every transport."""

    def published_outputs(self, plan: Any) -> Mapping[str, str | Path]: ...
